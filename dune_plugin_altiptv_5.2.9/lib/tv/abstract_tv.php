...
abstract class AbstractTv implements Tv
{
    ...
    public function get_tv_info(MediaURL $media_url, &$plugin_cookies)
    {
        ...
        foreach ($this->get_groups() as $g)
        {
			...
			if ($g->get_id()=='__all_channels')
				$n = 0;
			else{
				$key = array_search($g->get_title(), $group_list);
				if ($key === false)
					$n = $i;
				else
					$n = $key+1;
        ...
        (
            ...
            PluginTvInfo::favorites_icon_url => strval($this->get_fav_icon_url($plugin_cookies)),
			...
			PluginTvInfo::epg_font_size => isset($plugin_cookies->epg_font) ? $plugin_cookies->epg_font : PLUGIN_FONT_NORMAL,
    ...
    public function get_fav_channel_ids(&$plugin_cookies)
    {
		...
		if ($fav_save == 1){
			$fav_channel_ids = HD::get_items('favorite_channels', &$plugin_cookies);
		}
		else{
    ...
    public function set_fav_channel_ids(&$plugin_cookies, &$ids)
    {
		...
		if ($fav_save == 1){
			...
			HD::save_items('favorite_channels', $ids, &$plugin_cookies);
		}else
